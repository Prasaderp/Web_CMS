...
const Navbar = () => {
  ...
  // mobile-specific link styles (block-level so items stack cleanly on small screens)
      ...
      {isOpen && (
        <div className="lg:hidden bg-white border-t border-gray-200 shadow-lg max-h-[calc(100vh-70px)] overflow-auto">
          <div className="flex flex-col space-y-2 p-4">
            ...
            <Link to="/sign-in" className={mobileLinkClasses} onClick={() => setIsOpen(false)}>
              SIGN IN
            </Link>
            <Link to="/create-account" className={mobileLinkClasses} onClick={() => setIsOpen(false)}>
              CREATE ACCOUNT
            </Link>
            <Link to="/my-account" className={mobileLinkClasses} onClick={() => setIsOpen(false)}>
              MY ACCOUNT
            </Link>
          </div>
        </div>
